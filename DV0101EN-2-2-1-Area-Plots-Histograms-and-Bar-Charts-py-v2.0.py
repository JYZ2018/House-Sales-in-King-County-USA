...
df_top5_1 = df_can.head()
print(df_top5_1.head())

# transpose the dataframe
df_top5 = df_top5_1[years].transpose() 
...
df_bottom5_1=df_can.tail().loc[:,years]
#print(df_bottom5_1)
years=list(map(str,range(1980,2014)))
df_bottom5_2=df_bottom5_1.transpose()
print(df_bottom5_2.index)
df_bottom5_2.index=df_bottom5_2.index.map(int)
df_bottom5_2.index.name='Year'
print(df_bottom5_2)
df_bottom5_2.plot(kind='area',alpha=0.45)
plt.title('Bottom 5 countries taht immigrate to Canada')
plt.xlabel('years')
plt.ylabel('Number of immigrants')
plt.show()
...
ax=df_bottom5_2.plot(kind='area',stacked=False,alpha=0.55)
...
bins,edges=np.histogram(df_dns)
df_dns.plot(kind='hist',xticks=edges)
...
bins,edges=np.histogram(df_gab,15)
xmin=edges[0]-10
xmax=edges[-1]+10
df_gab.plot(kind='hist',stacked=False,alpha=0.35,bins=15,xlim=(xmin,xmax))
...
df_15=df_can['Total'].sort_values(ascending=False)[:15]
print(df_15)
...
df_15.plot(kind='barh',figsize=(12,12))
plt.title('Top 15 countries that have the most immigrants to Cananda from 1980 to 2013')
plt.xlabel('number of immigrants')
...
for index, value in enumerate(df_15):
    ...
    plt.annotate(label,xy=(value-47000,index-0.1),color='blue')
